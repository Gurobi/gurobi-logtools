...
class SingleLogBase(Parser):
    ...
    def __init__(self, write_to_dir=None):  # , objnlog=False):
    ...
    def get_progress(self, section):
        if section == "nodelog":
            progress = self.nodelog_parser.get_progress()
        elif section == "rootlp":
            progress = self.continuous_parser.get_progress()
        elif section == "norel":
            progress = self.norel_parser.get_progress()
        elif section == "pretreesols":
            progress = self.pretree_solution_parser.get_progress()
        else:
            raise ValueError(f"Unknown section '{section}'")
        progress.extend(self.multiobj_parser.get_objn_progress(section))
        return progress
    ...
    def parse(self, line: str) -> ParseResult:
        ...
        for i, parser in enumerate(self.future_parsers):
            if parse_result := parser.parse(line):
                ...
                if isinstance(parser, self._MultiObjParser):
                    self.future_parsers = []
                else:
